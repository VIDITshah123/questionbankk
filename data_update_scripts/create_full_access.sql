...
INSERT INTO base_roles_master (name, description, created_at, updated_at) 
...
INSERT INTO base_role_permissions_tx (role_id, permission_id, created_at)
SELECT 
    (SELECT role_id FROM base_roles_master WHERE name = 'full_access'), 
    permission_id,
    datetime('now')
FROM base_permissions_master;

-- Create FA user with password User@123
INSERT INTO base_users_master (mobile_number, email, password_hash, first_name, last_name, is_active, created_at, updated_at) 
...
INSERT INTO base_user_roles_tx (user_id, role_id, created_at)
VALUES (
    (SELECT user_id FROM base_users_master WHERE mobile_number = '8888888888'),
    (SELECT role_id FROM base_roles_master WHERE name = 'full_access'),
    datetime('now')
);
